    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="glass-card p-6 flex flex-col justify-between">
        ...
        <div className="space-y-3 mt-4">
          {[
            { name: "Poisson Model", status: "active", accuracy: 62 },
            { name: "XGBoost Ensemble", status: "active", accuracy: 71 },
            { name: "ELO System", status: "active", accuracy: 58 },
          ].map((m) => (
            <div key={m.name} className="space-y-1">
              <div className="flex justify-between text-xs">
                <span className="text-muted-foreground">{m.name}</span>
                <span className="text-neon-green">{m.accuracy}%</span>
              </div>
